    ...
    <div class="col-md-4 mb-4">
        <div class="card text-center">
            <div class="image-container">
                <img src="{{ asset('asset/default-image/' . $item->image_path) }}" alt="{{ $item->name }}" class="img-fluid">
            </div>
            <div class="card-body">
                <h5 class="card-title">{{ $item->name }}</h5>
                <p class="profit-text">
                    Profit: RM{{ number_format($item->total_quantity * $item->price, 2) }}
                </p>
            </div>
        </div>
    </div>
        ...
        <div class="card text-center">
            <div class="image-container">
                @if(auth()->user()->role === 'staff')
                    <img src="{{ asset('asset/default-image/' . $item->image_path) }}" alt="{{ $item->name }}" class="img-fluid">
                @else
                    <a href="{{ $item->status === 'Unavailable' ? '#' : route('customize.order', ['menu' => $item->id]) }}"
                       class="{{ $item->status === 'Unavailable' ? 'disabled-link' : '' }}">
                        <img src="{{ asset('asset/default-image/' . $item->image_path) }}" alt="{{ $item->name }}" class="img-fluid">
                    </a>
                @endif
            </div>
...
@foreach($newMenus as $menu)
    <div class="col-md-4 mb-4">
        <div class="card text-center">
            <div class="image-container">
                @if(auth()->user()->role === 'staff')
                    <img src="{{ asset('asset/default-image/' . $menu->image_path) }}" alt="{{ $menu->name }}" class="img-fluid">
                @else
                    <a href="{{ $menu->status === 'Unavailable' ? '#' : route('customize.order', ['menu' => $menu->id]) }}"
                       class="{{ $menu->status === 'Unavailable' ? 'disabled-link' : '' }}">
                        <img src="{{ asset('asset/default-image/' . $menu->image_path) }}" alt="{{ $menu->name }}" class="img-fluid">
                    </a>
                @endif
            </div>
            <div class="card-body">
                ...
                @if(auth()->user()->role === 'staff' && isset($menu->total_quantity))
                    <p class="profit-text">
                        Profit: RM{{ number_format($menu->total_quantity * $menu->price, 2) }}
                    </p>
                @endif
            </div>
        </div>
